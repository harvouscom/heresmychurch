...
const COUNTRIES = {
  CA: {
    admin: "Canada",
    source: "50m",
    // iso_3166_2 here is a real ISO 3166-2 code that Overpass resolves as an
    // area, so these regions can be ingested directly.
  ...
  GB: {
    ...
    // No Overpass area: the codes below are our own slugs for browsing, not ISO
    // 3166-2. Verified — area["ISO3166-2"="GB-SCOTLAND"] matches nothing, while
    // the real nation codes GB-SCT and GB-WLS return 3,195 and 3,124 churches.
    // Ingesting the UK therefore needs the four nations as areas, with these
    // regions assigned afterwards by point-in-polygon.
    overpassIso: null,
    groupBy: gbKey,
  ...
  IE: {
    admin: "Ireland",
    source: "10m", // not present at 50m
    // Ireland's ISO codes are real and Overpass resolves them, but four of them
    // collide with US state codes — IE-CO/KY/MN/MO against Colorado, Kentucky,
    // Minnesota and Missouri. KV keys are still flat `churches:{REGION}`, so
    // ingesting Cork would overwrite Colorado. Left out of the server table
    // until the country-namespaced migration runs; the boundary file below is
    // harmless and ready for that day.
    overpassIso: null,
    groupBy: (p) => p.iso_3166_2,
    code: (p) => p.iso_3166_2,
