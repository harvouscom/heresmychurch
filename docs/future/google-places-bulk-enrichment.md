...
Use the **Google Places API (new)** to enrich existing church records by matching each church to a Google Place and pulling Place Details. This doc focuses on **bulk enrichment** of the current OSM-sourced dataset; on-demand enrichment at add/edit time is a separate, smaller effort.

**How this fits the broader review strategy:** Places is an **optional feeder** for missing **address** and **website** so we can scrape sites and email churches to update listings. Under the **low-cost** posture in [faster-church-data-review.md](./faster-church-data-review.md), Places is **deferred (P3)** until denom backfill, scrape of existing websites, free website discovery, and church email outreach are measured — the ~$200/mo Maps credit is often not worth burning for needs-review gains.

**Constraint:** Do **not** map Google `regularOpeningHours` / business hours → `serviceTimes`. Office/building hours are not worship times. The live enrichment helper already never writes `serviceTimes` from hours (`google-places-enrichment.ts`). Prefer omitting hours from Place Details field masks.
...
- **Fields to enrich (from Place Details):** `address`, `city` (from address components), `phone`, `website`. Optionally overwrite `name` only when match confidence is high and the field is empty or clearly worse — treat as rare. **Not in scope:** `serviceTimes` from opening hours (rejected; see above).
...
| **Place Details** (new) | Get full details for a Place ID | Fetch formatted_address, geometry, name, international_phone_number, website. |

**Field masks** are critical for cost control: request only the fields you need (e.g. `displayName`, `formattedAddress`, `location`, `internationalPhoneNumber`, `websiteUri`). **Omit** `regularOpeningHours` — we do not use hours for `serviceTimes`. See [Place Details (New) field list](https://developers.google.com/maps/documentation/places/web-service/place-details#fields).
...
- For each church (optionally filter: e.g. missing address or website first):
  ...
  - If Place ID found: call Place Details with field mask for name, formattedAddress, location, internationalPhoneNumber, websiteUri (no opening hours).
  - Parse address into street + city + state (use address_components or split formattedAddress).
  - Write result to a stable key, e.g. `enrichment:google:${churchId}` with `{ placeId, fetchedAt, name?, address?, city?, state?, phone?, website? }`.
...
Recommendation: **A** for clarity (enrichments are a separate layer like corrections) and to avoid overwriting community-submitted data. Only overlay fields that are currently empty in the church record, or prefer enrichment only when the church has no correction for that field.
...
Optional: `confidence` (high / low) or `source` (text_search | nearby) for filtering or manual review. Do **not** store opening hours as `serviceTimes`.

### 6. After website fill — scrape and outreach (not Places hours)

Once a church has a website (from Places or otherwise), worship times and contact emails come from **website scrape** and **email the church to update their HMC listing** — not from Google hours. Full pipeline: [faster-church-data-review.md](./faster-church-data-review.md).
...
| `docs/future/faster-church-data-review.md` | Review-velocity strategy; Places as address/website feeder. |
| `supabase/functions/make-server-283d8046/index.ts` | Admin route `POST /admin/enrich-google/:state` (exists); merge enrichment into church response if using read-time merge. |
| `supabase/functions/make-server-283d8046/google-places-enrichment.ts` | Match, cache, fill-empty merge; never writes `serviceTimes` from hours. |
| KV | Keys: `enrichment:google:{churchId}`. Optional: `enrichment:no_match:{state}` as array of church ids. |
...
| `src/app/components/church-data.ts` | No change to `Church` type; address, phone, website already exist. |
...
4. **Ongoing:** Optional periodic refresh (e.g. re-fetch Place Details for last 10% of enriched churches per month) to keep phone/website/address up to date.
...
- Existing merge pattern: `getApprovedCorrectionsForState`, `mergeCorrectionsIntoChurches`; live helper in `supabase/functions/make-server-283d8046/google-places-enrichment.ts`
- Review-velocity strategy: [faster-church-data-review.md](./faster-church-data-review.md)
